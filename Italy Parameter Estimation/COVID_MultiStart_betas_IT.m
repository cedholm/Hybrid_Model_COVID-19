...
%
% This code calls the COVID_Model -- ODE equations
% 
...
% z = [ beta1, beta2, beta3, muI2, p, d1, g2] 

LowerBounds=[0.1, 0.001, 0.001, 0.01, 0.9, 0.07, 0.02];
UpperBounds = [2, 1, 1, 0.02, 0.95, 0.45, 0.03];
...
CumRecov=Data(:,4)';
TotalPop=60000000;
InitialExposedSilent=300;
InitialAsymptomaticSilent=300;
InitialRecoveredSilent=0;
InitialExposedSymptomatic=25;
InitialAsymptomaticSymptomatic=0;
InitialRecoveredSymptomatic=0;

elseif bc == 1                                          %% British Columbia Data
Data=readtable('BC200611_WithRecover.xlsx','Range','C1:G111');
Data=table2array(Data);
CumCases=Data(:,1)';
CumDeaths=Data(:,2)';
TimeSteps=Data(:,3)';
...
CumRecov=Data(:,5)';
TotalPop=5110917;
InitialExposedSilent=3;
InitialAsymptomaticSilent=2;
InitialRecoveredSilent=0;
...
header={'Exit Flags','fvalues','beta1', 'beta2', 'beta3', 'muI2', 'p', 'd1', 'g2'};
...
p=z(5);                                                 %fraction of the population that are SilentSpreader
...
[t,y] = ode45(@(t,y) COVID_Model_betas_IT(t,y,z),tspan,initialvalues);
...
%R0= 2.38;
%beta3= ((R0-(beta1*p)/(delta1)-(beta2*(1-p))/(delta2))*((gamma2+mu2)))/(1-p);
%beta3=((R0-(beta1*p)/delta1-(beta2*(1-p))/delta2)*(gamma2+mu2))/(1-p);
%beta3=((R0-((beta1*p)/delta1)-((beta2*(1-p))/delta2))*((gamma2+mu2))/(1-p));
fprintf('Ro=%s\n',R0);
